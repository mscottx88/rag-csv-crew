...
class TestColumnEmbeddings:
    ...
    def test_embeddings_generated_on_csv_upload(
        ...
        service: IngestionService = IngestionService(mock_conn)
    ...
    def test_embeddings_stored_with_column_mappings(
        ...
        - Queryable via pgvector operators
        """
        ...
        ingestion_service: IngestionService = IngestionService(test_db_connection)

        # Create column mapping with embedding
        column_name: str = "total_sales"
        column_description: str = "Sum of all sales transactions"
        embedding_text: str = f"{column_name} {column_description}"

        embedding: list[float] = vector_service.generate_embedding(embedding_text)

        # Store in database
        ingestion_service.store_column_mapping(
            username="testuser",
            dataset_id="test-dataset-id",
    ...
    def test_embedding_generation_failure_rollback(self) -> None:
        """Test transaction rollback when embedding generation fails.
        ...
        Success Criteria (T099):
        - Failed embedding generation doesn't leave orphaned data
