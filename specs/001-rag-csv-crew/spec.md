...
- **FR-024**: System MUST maintain audit logs for all significant operations in structured JSON format for system observability and debugging (these audit logs are separate from user-facing query history in FR-011; "audit logs" = system diagnostic logs for operators/developers, "query history" = user-accessible past queries and results for end users), with mandatory fields (timestamp ISO 8601, log_level, event_type, username, request_id) and event-specific fields (execution_time_ms for queries, row_count for uploads, error_message and stack_trace for failures), covering user authentication events (login, logout, session expiration), file operations (upload start/complete/fail, delete, conflict resolution), query lifecycle (submission, execution start, completion, cancellation, timeout), database operations (connection retries, schema creation), errors, and security-relevant actions (cross-schema access attempts, rate limit violations)
